...
class Pm(object):
    ...
    #
    #  @param pmlist the array of new pitch-marks to set (array of OnePm objects or (time, type) tuples to be passed
    #         through pitch-mark factory callable set through the constructor)
    #  @param name optional name of the sequence
    #  @param self
    #
    def set_pmks(self, pmlist, name = None):
    ...
    def del_duplicates(self, toler = None):
        self.logger.debug('Deleting duplicated pitch-marks (>{} Hz) in {} pitch-marks'.format(toler, len(self)))
    ...
    def find_isolates(self, t0_threshold):
        isos = []
        self.logger.debug('Finding isolated pitch-marks (>{} s) in {} pitch-marks'.format(t0_threshold, len(self)))
        try:
            if self[1].time - self[0].time > t0_threshold:
                isos.append(1)
        except IndexError:
            self.logger.warning('At least two pitch-marks expected! Is there any speech?')
            return isos
    ...
    def del_isolates(self, t0_threshold):
        self.logger.debug('Delete isolated pitch-marks (>{} s) in {} pitch-marks'.format(t0_threshold, len(self)))
        for idx in reversed(self.find_isolates(t0_threshold)):
            self.logger.debug('Pitch-mark {} deleted'.format(self[idx]))
            del self[idx]
    ...
    def find(self, time, types = {OnePm.type_V, OnePm.type_U}, idx_start = 0):
        ...
        if   self.__shift_type == Pm.shift_nearest :
             ...
             raise ValueError('No nearest pitch-mark found to time {} for types {} (file {})'.format(time, types, self.__fname))
    ...
    def shift_idx(self, idx_start, steps, skip_T = True) :
        ...
        while idx_start != end :
            ...
            # Out of range
            if idx_start < 0 :
                raise IndexError('Pitch-marks sequence underflow')
            if idx_start >= len(self) :
                raise IndexError('Pitch-marks sequence overflow')
    ...
    #
    def __repr__(self):
        return '[Array of {} pitch-marks, read from: {}]'.format(len(self.__pmlist), self.__fname)
